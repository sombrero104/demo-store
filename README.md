...
- JWT + Spring Security로 인증/인가 처리 (Refresh Token은 Redis로 관리) 
- 주문 시 Redisson 분산 락으로 재고 정합성 보장
- AI intent 분류로 자연어 요청을 주문 조회/취소로 연결
- AI API 호출에 지수 백오프와 jitter를 적용한 재시도 사용
<br/>

## 기술 스택
- Backend: Spring Boot, Spring Security, Spring Data JPA, Querydsl
- Database: MySQL
- Infra / Tooling: Redis(Refresh Token 저장, Redisson 기반 재고 동시성 제어), Flyway, Swagger
- AI: LLM(OpenAI, Gemini) 기반 자연어 intent 분류 및 요청 분기 처리
- Test: JUnit5

> Java 기반 프로젝트를 Kotlin으로 점진적으로 일부 모듈부터 전환 중
